<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="案例编号" prop="anlibianhao">
            <el-input v-model="ruleForm.anlibianhao" 
                placeholder="案例编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.anlibianhao" label="案例编号" prop="anlibianhao">
              <el-input v-model="ruleForm.anlibianhao" 
                placeholder="案例编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="案例详情" prop="anlixiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.anlixiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.anlixiangqing" label="案例详情" prop="anlixiangqing">
                    <span v-html="ruleForm.anlixiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="案例分析" prop="anlifenxi">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.anlifenxi" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.anlifenxi" label="案例分析" prop="anlifenxi">
                    <span v-html="ruleForm.anlifenxi"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.anlixiangqing = this.ruleForm.anlixiangqing.replace(reg,'../../../springbootsme07/upload');
	this.ruleForm.anlifenxi = this.ruleForm.anlifenxi.replace(reg,'../../../springbootsme07/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `anlifenxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.anlifenxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
